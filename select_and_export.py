...
STATE = Path(r'd:\vscode\1688_auto_trial\shadow_select_export.txt')
...
with sync_playwright() as p:
    ...
    () => {
      ...
      const target = Array.from(host.shadowRoot.querySelectorAll('label,span,button,div'))
        .find((el) => normalize(el.textContent).includes('有销量['));
      if (!target) return 'NO_SALES_FILTER';
      target.click();
      return normalize(target.textContent);
    }
    ''')
    logs.append(f'SELECT={select_state}')
        ...
        () => {
          ...
          for (const root of roots) {
            ...
            for (const el of els) {
              ...
              if (!t) continue;
              if (t.includes('导出') || t.includes('设置') || t.includes('字段') || t.includes('标题') || t.includes('所在地') || t.includes('商品链接') || t.includes('主图链接') || t.includes('上架时间')) {
