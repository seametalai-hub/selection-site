...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Auto-filter 1688 inventory page, then scrape the first 30 products."
    )
    parser.add_argument("--url", default=TARGET_URL, help="Target 1688 page URL.")
    parser.add_argument(
        "--output",
        default="1688_auto_products_page1.csv",
...
def click_text(page, text: str, timeout: int, *, exact: bool = True) -> None:
...
def click_if_visible(page, text: str, timeout: int, *, exact: bool = True) -> bool:
    locator = page.get_by_text(text, exact=exact).first
    try:
...
def apply_auto_filters(page, timeout: int) -> None:
    page.wait_for_timeout(1500)
    click_if_visible(page, "更多类目", timeout)
    page.wait_for_timeout(500)

    click_text(page, "汽车用品", timeout)
    page.wait_for_timeout(800)

    click_text(page, "座垫脚垫", timeout)
    page.wait_for_timeout(800)

    click_text(page, "近7天上新", timeout)
    page.wait_for_timeout(800)
...
def main() -> int:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            apply_auto_filters(page, args.timeout)
